...
| 2026-02-15 | Shared Folder Configuration – Students OU Access Control | Created a centralized shared folder for Students with read-only access by configuring NTFS and share permissions through a dedicated security group. Organized all student accounts into the group to ensure scalable and controlled access to shared academic resources. |
...
| 2026-04-11 | Network Troubleshooting Documentation Start | Started with creating documentation for demonstrating how to resolve issues where users don't have access to the internet. Simulated a common problem in client machine VM where the wrong default gateway is entered, which doesn't allow external websites to be connected to the user. |
| 2026-04-19 | Server 2022 Extend License | Extended the evalulation license for the Windows Server 2022 machine that serves as the Domain Controller for my homelab. Used Powershell terminal to run the command: slmgr /rearm, to extend the license for another 180 days to avoid having to restart setting up domain controller. |
